...
We used ```Racon v1.5.0```. In order to use Racon, one needs to first map the sequencing reads to the assembly to be polished. You can use Racon with either ONT data or Illumina, but I chose to do it with ONT data. So I used ```minimap2 v. 2.26-r1175``` to map the ONT reads to the assembly
...
Once mapped, I proceeded to actually using Racon:
...
One of the other polishing software is Pilon. According to the authors:
...
To use Pilon it is of unparallel importance to map the reads, be it Nanopore or Illumina, to the assembly. I chose to polish the genome with Illumina, therefore I used ```bwa mem v. 0.7.17-r1188``` to the mapping step. 
...
Now, we can proceed to actually use Pilon. I used ```Pilon v. 1. 24```, available as a module on ```biotec02.esalq.usp.br```
...
After polishing the Canu assembly, it is possible to see that the polishing did not have any effect whatsoever on completeness, no matter the software or number of iterations. P.S: I only did more than on iteration with Pilon. I also did not use Pilon by itself. 
